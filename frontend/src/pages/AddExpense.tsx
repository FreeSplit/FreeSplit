...
const AddExpense: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let attempts = 0;
    const maxAttempts = 50; // Try for up to ~1 second
    ...
    const applyHostStyles = () => {
      ...
      if (host) {
        ...
        // Try to access and style the shadow root content
        try {
          const shadowRoot = host.shadowRoot;
          if (shadowRoot) {
            // Inject styles into shadow DOM to fix layout and enable tabs
            ...
            style.textContent = `
              ...
              /* Fix tabs not working - ensure they receive pointer events */
              [role="tablist"] {
              ...
              /* Ensure tab content is also centered */
              [role="tabpanel"] {
                display: flex !important;
                justify-content: center !important;
              }
            ...
            const existingStyle = shadowRoot.querySelector('style[data-custom-emoji-styles]');
            if (!existingStyle) {
              style.setAttribute('data-custom-emoji-styles', 'true');
